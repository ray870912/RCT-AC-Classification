...
# path = "/Users/ray/Thesis/Shoulder/External Rotation_120/"
...
for fileName in allFileList:
  ...
  if not fileName.startswith('.') and os.path.isfile(os.path.join(path, fileName)):
    ...
    while success:
      ...
      if success == True:
        ...
        # cv2.imwrite("/Users/ray/Thesis/Input/output/"+str(fileName[0:6])+str(countFrames)+".jpg", image)
  ...
  if continueProc == True:
    ...
    startFrame = utils.getStartPosition(utils.PositionY[0:30, wrist])

    # End Position
    stbEndFrame = utils.getEndPosition(utils.PositionX[30:, wrist],
                                       utils.PositionY[30:, wrist]) # Keep detecting highest point
    
    # Reduce Data Size -> normalize to 600 Frames
...
np.savetxt("exRotation_pro.csv", allFeature, fmt="%s", delimiter=",", header=header)