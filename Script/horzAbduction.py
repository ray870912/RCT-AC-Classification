...
path = "/Users/ray/Thesis/Shoulder/Horizontal Abduction_80/"
...
for fileName in allFileList:
  ...
  if not fileName.startswith('.') and os.path.isfile(os.path.join(path, fileName)):
    ...
    bodylength = math.sqrt((X[hip]-X[shoulder])**2 + (Y[hip]-Y[shoulder])**2)

    # Have some problem --> Need to re-Write this "if" statement
    if Y[hip] < Y[shoulder]:
        direction = 'flip'
    # if Y[hip] > Y[shoulder]:
    #   direction = 'normal'
    ...
    # elif X[hip]*1.5 < X[shoulder]:
    #   direction = 'counterClockwise_rotation'
    ...
    while success:
      ...
      elif direction == 'clockwise_rotation':
        image = cv2.rotate(image, cv2.ROTATE_90_CLOCKWISE)
      elif direction == 'counterClockwise_rotation':
  ...
  if continueProc == True:
    if side == 'right_':
      ...
      stbEndFrame = utils.getEndPosition(utils.PositionX[30:, wrist],
                                         ...
                                         'X', 'max') # Keep detecting highest point
    elif side == "left_":
    ...
    startShoulder = [utils.PositionX[startFrame][shoulder], utils.PositionY[startFrame][shoulder]]
    startElbow = [utils.PositionX[startFrame][elbow], utils.PositionY[startFrame][elbow]]
    ...
    if StartEndFrames >= 60:
      ...
      for i in splitList:
        ...
        utils.getHorzAngle(startShoulder, startElbow, currentShoulder, currentElbow)
...
np.savetxt("horzAbduction_dis.csv", allFeature, fmt="%s", delimiter=",", header=header)